...
namespace Program {
  class BenchmarkThreading {
    ...
    public object[][] BenchSort(Constants.ALGORITHM typeAlgorithm, int[][] arrays, bool debug) {
      ...
      SolverThreading<int[], int[]> algorithm = new SolverThreading<int[], int[]>(new MergeSort<int>());
      switch (typeAlgorithm) {
        case Constants.ALGORITHM.QuickSort:
          algorithm = new SolverThreading<int[], int[]>(new QuickSort<int>());
          break;
      }
    ...
    public object[][] BenchSearch(Constants.ALGORITHM typeAlgorithm, Search<int>[] arrays, bool debug) {
      int result;
      object[][] timeResults = new object[arrays.Length][];
      SolverThreading<Search<int>, int> algorithm = new SolverThreading<Search<int>, int>(new BinarySearch<Search<int>, int>());
      Stopwatch sw = new Stopwatch();
      for (int i = 0; i < arrays.Length; i++) {
        sw.Reset();
        sw.Start();
    ...
    public object[][] BenchTower(Constants.ALGORITHM typeAlgorithm, Tower[] arrays, bool debug) {
      object[][] timeResults = new object[arrays.Length][];
      Stopwatch sw = new Stopwatch();
      SolverThreading<Tower, Step[]> algorithm = new SolverThreading<Tower, Step[]>(new HanoiTower());
      for (int i = 0; i < arrays.Length; i++) {
        sw.Reset();
        sw.Start();
